...
def main(page: ft.Page):
    ...
    promt = ft.TextField(label="Escribe tu mensaje...", expand=True, multiline=True, min_lines=1, max_lines=4)
    
    def burbuja(texto, es_ususario):
         return ft.Row(
             [
                 ft.Container(
                     content=ft.Text(
    ...
    page.add(
        ft.Colum([
            ...
            mensajes,
            ft.Row([promt, boton_enviar], veretical_alignment=ft.CrossAxisAligment.END),        
        ], expand=True)
    )
